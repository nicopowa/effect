...
class Effect {
	...
	static addEffect(effect) {
		...
		if(!this.frameLoop) { // not running, lift off
			if(DEBUG) console.log("start frame loop");
	...
	static animationFrame(stamp) {
		if(!this.effects.length) {
			if(DEBUG) console.log("end frame loop");
			return this.frameLoop = 0; // pool is empty, stop frame loop
		}
		...
		while(this.done.length) { // callbacks
	...
	static effectTick(res, eff, step) {
		if(--eff.later > 0) res.push(eff); // delayed, wait
			...
			else this.done.push(eff); // done, resolve + callback and don't push
		}
		return res;
	}
