...
async function simpleEffect() {
	...
	let dur = 60;
	let maxLeft = Math.min(window.innerWidth - 100 - 10, 500) + "px";
	...
	new Effect(sq4, {"frames": dur * 1.5, "props": {"left": Math.min(700, document.body.clientWidth - 100 - 10)}, "ease": "no"}).play();
...
async function overrideTest() {
	...
	let eff = new Effect(sq1, {frames: 180, props: {left: "500px"}, ease: "quartInOut"});
	eff.play();
	console.log("wait");
	await plz(1000);
	//console.log("override");
	//await new Effect(sq1, {frames: 90, props: {left: "10px"}, ease: "quartInOut", override: true}).play();
	console.log("stop");
	eff.stop();
	console.log("done");
}